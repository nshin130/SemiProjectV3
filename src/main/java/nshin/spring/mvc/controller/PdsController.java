...
import nshin.spring.mvc.service.PdsService;
import nshin.spring.mvc.utils.FileUpDownUtil;
...
//    }
    ...
    public void pdsdown(String pno, String order, HttpServletResponse res) {
        ...
        Pds p = psrv.readOneFname(pno, order);
        // 다운로드 할 파일 정보 알아냄
        psrv.downCountPds(pno, order);
        // 다운로드 한 파일 다운수 증가
        fud.procDownload(p.getFname1(), p.getUuid(), res);
        // 다운로드 처리
